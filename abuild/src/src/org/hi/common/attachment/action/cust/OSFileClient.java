...
public class OSFileClient{
	...
	public String saveFile(File file, String fileName,String moduleName, String contextPath) throws IOException {
		...
		String formatedFileName = this.CreateTimeFileName(filePath,fileName);
	 ...
	 * @return
	 */
	private String CreateTimeFileName(String filePath,String fileName){
		String preName = "";
		String extName = "";
		String formatedFielName  = fileName;
		if (fileName.indexOf(".") < 0 )
		...
		preName=(new Date()).getTime()+org.hi.common.util.StringUtils.randomString(5);//时间+随机字符
		int i=1;
		formatedFielName=(new Date()).getTime()+org.hi.common.util.StringUtils.randomString(5)+extName;
		File file  = new File (filePath + formatedFielName );
		while (file .exists() )
		{
			formatedFielName = preName+  + i + extName;
			i ++;
