	...
	<div class="panelBar">
		...
		<c:choose>
			<c:when test="${empty lookup}">
				<authz:authorize ifAnyGranted="MEMBER_SAVE"><li><a class="add" href="<hi:url>memberEdit.action?member.id=-1</hi:url>" target="navTab" rel="member"><span><hi:text key="新建" parameterLanguageKeys="会员列表"/></span></a></li></authz:authorize>
				<authz:authorize ifAnyGranted="MEMBER_DEL"><li><a class="delete" href="<hi:url>memberRemoveAll.action?ajax=1</hi:url>" target="removeSelected" title="<hi:text key="确实要删除这些记录吗?"/>"><span><hi:text key="批量删除"/></span></a></li></authz:authorize>
			</c:when>
			<c:otherwise>
	...
	<table class="table" width="100%" layoutH="138" targetType="${targetType}">
		...
		<tbody>
			...
			<tr>
				...
				<c:choose>
					<c:when test="${empty lookup}">	
				    <authz:authorize ifAnyGranted="MEMBER_DEL">
				    ...
				    <authz:authorize ifAnyGranted="MEMBER_VIEW">
				    ...
				    <authz:authorize ifAnyGranted="MEMBER_SAVE">
